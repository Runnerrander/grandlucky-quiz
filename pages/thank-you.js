...
export default function ThankYou() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      setLoading(false);
      setErr(
        "Hiányzó PayPal token. Ne nyisd meg közvetlenül ezt az oldalt. / Missing PayPal token."
      );
      return;
    }
    ...
    (async () => {
      try {
        ...
        console.error(e);
        setErr(
          "Váratlan hiba történt a fizetés feldolgozásakor. / Unexpected error."
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <>
      <Head>
        <title>Köszönjük / Thank you — GrandLuckyTravel</title>
        <meta name="robots" content="noindex" />
      </Head>

      <main style={{ maxWidth: 760, margin: "40px auto", padding: 24 }}>
        <h1>Köszönjük a fizetést! / Thank you for your payment!</h1>
